...
export class ChampionsService {
  ...
  async getChampions(
    ...
    offset?: number,
    season?: string
  ): Promise<SeasonChampionsResponse> {
    const cacheKey = `champions:${season || 'all'}:${limit}:${offset}`;

    try {
      // Try to get from cache first
      const cachedData: null = await this.cache.get(cacheKey);
  ...
  private async getChampionsFromDatabase(
    ...
    // Define type for champion with includes
    type ChampionWithIncludes = (typeof champions)[0];
    ...
    return {
      MRData: {
        ...
        StandingsTable: {
          season,
          StandingsLists: champions.map((champion: ChampionWithIncludes) => ({
